...
def render_court_info(case_data: Dict):
    ...
    with col2:
        ...
        # Handle different types of court_time
        if isinstance(court_time, datetime):
            court_time_value = court_time.time()
        elif isinstance(court_time, time):
            # It's already a time object
            court_time_value = court_time
        elif isinstance(court_time, str) and court_time:
            court_time_value = datetime.strptime(court_time, "%H:%M").time()
        else:
            court_time_value = None
        
        case_data['court_time'] = st.time_input(
            "Court Time",
            value=court_time_value,
            key="court_time"
        )
